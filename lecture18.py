...
# let rec fold (f: 'a -> 'b -> 'b) (v : 'b) (l: 'a list) : b =
#   match l with
#   | [] -> v
#   | h :: t -> f h (fold f v t)
# 
# let sum_list (l: int list) : int = fold_right (fun x y -> x + y) 0 l


# One can write it in Python as: 
...
def fold_right(f: Callable[[A, B], B], v: B, l: list[A]) -> B:
    match l:
        case []: return v
        case [h, *t]: return f(h, fold_right(f, v, t))

def sum_list(l: list[int]) -> int:
    return fold_right(lambda x, y: x + y, 0, l)

# However, this is not why you use Python. Python gives us liberties to write 'simpler' code. We can write:

def fold_right(f, v, l):
    if (l == []):
        return v
    else:
        return f(l[0], fold_right(f, v, l[1:]))
